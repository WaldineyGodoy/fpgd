      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-1 space-y-6">
          <div className="bg-white p-6 rounded-[2.5rem] border border-slate-100 shadow-sm relative overflow-hidden">
            ...
            <p className="text-xs font-bold text-slate-400 uppercase tracking-widest relative z-10">Capacidade da Usina</p>
            <div className="mt-8 relative z-10 flex items-baseline gap-2">
              <span className="text-5xl font-black text-green-600">{usina.potencia_usina || 0}</span>
              <span className="text-lg font-bold text-slate-400">kWp</span>
            </div>
          </div>

          <div className="bg-white p-6 rounded-[2.5rem] border border-slate-100 shadow-sm">
            <h3 className="text-lg font-black text-slate-800 flex items-center gap-2 mb-6">
              <Settings className="w-5 h-5 text-blue-500" /> Equipamentos
            </h3>
            <div className="space-y-4">
               <div className="flex justify-between items-center p-3 bg-slate-50 rounded-xl">
                 <span className="text-xs font-bold text-slate-500 uppercase">Qtd. Painéis</span>
                 <span className="font-black text-slate-700">{usina.qtd_paineis || 0} un</span>
               </div>
               <div className="flex justify-between items-center p-3 bg-slate-50 rounded-xl">
                 <span className="text-xs font-bold text-slate-500 uppercase">Pt. Painéis (W)</span>
                 <span className="font-black text-slate-700">{usina.potencia_paineis || 0} W</span>
               </div>
               <div className="flex justify-between items-center p-3 bg-slate-50 rounded-xl">
                 <span className="text-xs font-bold text-slate-500 uppercase">Qtd. Inversores</span>
                 <span className="font-black text-slate-700">{usina.qtd_inversor || 0} un</span>
               </div>
               <div className="flex justify-between items-center p-3 bg-slate-50 rounded-xl">
                 <span className="text-xs font-bold text-slate-500 uppercase">Pt. Inversor (kW)</span>
           ...
           <div className="bg-white p-6 rounded-[2.5rem] border border-slate-100 shadow-sm">
            <h3 className="text-lg font-black text-slate-800 flex items-center gap-2 mb-6">
              <Hash className="w-5 h-5 text-slate-500" /> Identificação e Dados
            </h3>
            <div className="space-y-4">
              <div>
             ...
             <div className="flex justify-between items-start mb-8">
               <div>
                  <h3 className="text-xl font-black text-slate-800 flex items-center gap-2">
                    <BarChartIcon className="w-6 h-6 text-green-500" /> Gráfico de Geração
                  </h3>
                  <p className="text-xs font-bold text-slate-400 uppercase mt-1 tracking-widest">
                    Projeção Estimada Mensal (kWh)
                  </p>
               </div>
               <div className="text-right">
                  <span className="block text-2xl font-black text-green-600">{usina.geracao_media_anual || 0}</span>
                  <span className="text-[10px] font-bold text-slate-400 uppercase">Total Anual Estimado</span>
               </div>
             </div>
                ...
                {usina.geracao_media_anual ? (
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={chartData}>
                      ...
                      />
                      <Bar dataKey="geracao" fill="#22c55e" radius={[6, 6, 0, 0]} />
                    </BarChart>
                  </ResponsiveContainer>
                ) : (
